...
class IndexController extends BaseController
{
    ...
    public function index()
    {
        ...
        $this->assignSelf();
        //if (is_login() && check_auth('Weibo/Index/doSend')) {  zouj modify
        if (is_login()){
    ...
    public function doSend()
    {
        ...
        if (!in_array($aType, $types)) {
            ...
            if (!$class_exists) {
                ...
                if (method_exists($class, 'parseExtra')) {
                    if($aType == 'xiami') {
                        $data = getXiaMiUrl($aExtra['id']);
                        if($data) {
                            $aExtra['src'] = ipcxiami($data['location']);
                        }
                    }
        ...
        //权限判断
        $this->checkIsLogin();
        //$this->checkAuth(null, -1, L('_INFO_AUTHORITY_LACK_'));  zouj delete
    ...
    public function doSendRepost()
    {
        ...
        //$this->checkAuth(null, -1, L('_INFO_AUTHORITY_TRANSMIT_LACK_')); zouj  modify
