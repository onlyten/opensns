...
class IndexController extends BaseController
{
    ...
    private function group_opt($aKeyword=""){
        ...
        /****zouj add start****/
            ...
            $param['where']['school_id'] = $school_id;
        /****zouj add end****/
    ...
    public function select()
    {
        ...
        $groups = $groupModel->field('id')->order('rand()')->where(array('status' => 1,'school_id'=>$school_id))->limit(15)->select();
        foreach ($groups as &$v) {
    ...
    {  
        ...
        /****zouj add end****/
        $groupModel = D('Group');
        $group_ids = $groupModel->where(array('status' => 1,'school_id'=>$school_id))->field('id')->select();
    ...
    public function create()
    {
        if (IS_POST) {
            ...
            if ($isEdit) {
                $data = array('id' => $aGroupId, 'school_id' => $school_id, 'type_id' => $aGroupType, 'title' => $aTitle, 'detail' => $aDetail, 'logo' => $aLogo, 'type' => $aType, 'background' => $aBackground, 'member_alias' => $aMemberAlias);
                $data['status'] = $need_verify ? 0 : 1;
                $result = $model->editGroup($data);
                $group_id = $aGroupId;
            } else {
                $data = array('type_id' => $aGroupType,'school_id' => $school_id, 'title' => $aTitle, 'detail' => $aDetail, 'logo' => $aLogo, 'type' => $aType, 'uid' => is_login(), 'background' => $aBackground, 'member_alias' => $aMemberAlias);
                $data['status'] = $need_verify ? 0 : 1;
                ...
                /****zouj add start ****/
                if (I("post.teac")) {//群组里面添加的老师
                    $arr_teac = I("post.teac");
                    $num = count($arr_teac);
                    for($i=0;$i<$num;$i++){
                        D('GroupMember')->addMember(array('uid' => $arr_teac[$i], 'group_id' => $group_id, 'status' => 1, 'position' => 1));
                    }
                }
                if (I("post.stu")) {//群组里面添加的学生
                    $arr_stu = I("post.stu");
                    $num = count($arr_stu);
                    for($i=0;$i<$num;$i++){
                        D('GroupMember')->addMember(array('uid' => $arr_stu[$i], 'group_id' => $group_id, 'status' => 1, 'position' => 1));
                    }
                }
                if (I("post.par")) {//群组里面添加的家长
                    $arr_par = I("post.par");
                    $num = count($arr_par);
                    for($i=0;$i<$num;$i++){
                        D('GroupMember')->addMember(array('uid' => $arr_par[$i], 'group_id' => $group_id, 'status' => 1, 'position' => 1));
                    }
                }
                /****zouj add end****/
            }
            ...
            if(!$num){
                $this->error("您没有创建群组的权限！");
            }
            $username = session("username");
            $mapaa["nickname"] = $username;
            $typeone = M("member")->where($mapaa)->getField("type");
            $school_id = M("member")->where($mapaa)->getField("school_id");
            $teac_id = M("member")->where($mapaa)->getField("teacher_id");
            $mapzo["teacher_id"] = $teac_id;
            $class_id = M("ref_teacher_class")->where($mapzo)->getField("class_id",true);
            $pai["uid"] = array("neq",is_login());//排除自身
            $num = count($class_id);
            for($i=0;$i<$num;$i++){
                $mapon["class_id"] = array("eq",$class_id[$i]);
                $student[] = M("member")->where($mapon)->where($pai)->field("uid,nickname")->select();//所有的学生
            }
            $department = M("org_department")->where("school_id=".$school_id)->field("id,name")->select();//一个学校所有的部门
            $num = count($department);
            for($i=0;$i<$num;$i++){
                $maptw["department_id"] = array("eq",$department[$i]['id']);
                $teacher[] = M("user_teacher")->where($maptw)->field("id,name")->select();//所有的老师
            }
            $num = count($teacher);
            for($i=0;$i<$num;$i++){//从member表中找到老师对应的uid
                $numm = count($teacher[$i]);
                for($j=0;$j<$numm;$j++){
                    $teacher[$i][$j]['uid'] = M("member")->where("teacher_id=".$teacher[$i][$j]['id'])->getField('uid');
                    if($teacher[$i][$j]['uid'] == is_login()){//排除自身
                        unset($teacher[$i][$j]);
                    }
                }
            }
            ...
            for($i=0;$i<$num;$i++){
                $numm = count($parent[$i]);
                for($j=0;$j<$numm;$j++){
                    $student_id = M("member")->where("uid=".$parent[$i][$j]['uid'])->getField("student_id");
                    $parent[$i][$j]['parent_name'] = M('user_patriarch')->where("student_id=".$student_id)->getField('name');
                    $nn["nickname"] = $parent[$i][$j]['parent_name'];
                    $parent[$i][$j]['uid'] = M("member")->where($nn)->getField('uid');
                }
            }
            // dump($student);
            // dump($parent);
            // die();
            $this->assign("parennnt",$parent);
            $this->assign("department",$department);
            $this->assign("student",$student);
            $this->assign("teacher",$teacher);
            //$this->assign("num",510/($num+1));
            /****zouj add end****/
            $this->requireLimit();
            $this->requireLogin();
    ...
    public function my()
    {
        ...
        $myattend = $groupModel->getList(array('where' => array('id' => array('in', $group_ids), 'status' => 1,'school_id'=>$school_id), 'page' => $aPage, 'count' => $r, 'order' => 'uid = ' . is_login() . ' desc ,uid asc'));
    ...
    public function search($page = 1, $uid = 0)
    {
        ...
        /****zouj add start****/
            ...
            $select_id = M("group")->where($mmm)->getField('id',true);
            $num = count($list);
            for($i=0;$i<$num;$i++){
                if(in_array($list[$i]['group_id'],$select_id)){
                    $arr[] = $list[$i]; 
                }
            }
            unset($list);
