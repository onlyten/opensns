...
class ForumController extends AdminController
{
    ...
    public function editForum($id = null, $title = '', $create_time = 0, $status = 1, $allow_user_group = 0, $logo = 0, $type_id = 0,$school_id = 0)
    {
        if (IS_POST) {
            //判断是否为编辑模式
            ...
            if (I('quick_edit', 0, 'intval')) {
                ...
                //生成数据
                $data = array('title' => $title, 'school_id'=>$school_id, 'create_time' => $create_time, 'status' => $status, 'allow_user_group' => $allow_user_group, 'logo' => $logo, 'admin' => I('admin', 1, 'text')
            ...
            /****zouj add start ****/
            $school = M("org_school")->field("id,name")->select();
            $num = count($school);
            $arr[0] = "全国";
            for($i=0;$i<$num;$i++){
                $arr[$school[$i]['id']] = $school[$i]['name'];
            }
            /****zouj add end****/
            //显示页面
            ...
            $builder
                ...
                ->keySelect('school_id', "选择学校","", $arr)//zouj add 
