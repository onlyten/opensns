...
/****zouj add start****/
        ...
        $mapa["nickname"] = $username;
        $school_id = M("member")->where($mapa)->getField("school_id",true);
        $school_id[1] = 0;
        $mapb['school_id'] = array("in",$school_id);
        $forum_id = M("forum")->where($mapb)->getField("id",true);
        $mapc["forum_id"] = array("in",$forum_id);
/****zouj add end****/
        $map["school_id"] = M("member")->where("uid=".is_login())->getField("school_id");//zouj add 
        $list = D('ForumPost')->where($map)->where($mapc)->order($aOrder)->page($aPage, modC('FORM_POST_SHOW_NUM_INDEX',5,'Forum'))->select();
    ...
    public function forum()
    {
        if(I("get.")){//拿到get参数
            $arr = I("get.");
            $all_key = array_keys($arr);//所有的key值
            $str = "sns!@#$%".$all_key[1];//需要加密的字符串
            $md_str = md5($str);
            if($md_str == $arr[$all_key[1]]){//判断密钥是否一致
                $username = $all_key[1];
                $map["nickname"] = $username;
                $num = M("member")->where($map)->count();
                if($num == 0){
    ...
    public function doEdit($post_id = null, $forum_id = 0, $title, $content)
    {
        ...
        //写入帖子的内容
        $school_id = M("member")->where("uid=".is_login())->getField("school_id");//zouj add
        $model = D('ForumPost');
        if ($isEdit) {
            ...
            $data = array('uid' => is_login(), 'title' => $title, 'content' => $content, 'parse' => 0, 'school_id'=> $school_id, 'forum_id' => $forum_id);
...
/****zouj add start****/
        ...
        $school_id = M("member")->where($mapaa)->getField("school_id",true);
        $school_id[1] = 0; 
        $mapbb['school_id'] = array("in",$school_id);
        $forum_id = M("forum")->where($mapbb)->getField("id",true);
        $str = implode(",",$forum_id);
        $strr = "(".$str.")";
/****zouj add end****/
        $prefix = C('DB_PREFIX');
        $sql="select * from {$prefix}forum_post where status=1 and forum_id in $strr order by rand() LIMIT 0 , ".modC('FORM_POST_SHOW_NUM_PAGE','10','Forum');
        $post = M('')->query($sql);
...
/****zouj add start****/
        ...
        $mapbb['school_id'] = array("in",$school_id);
        $forum_id = M("forum")->where($mapbb)->getField("id",true);
        $where['forum_id'] = array("in",$forum_id);
/****zouj add end****/
