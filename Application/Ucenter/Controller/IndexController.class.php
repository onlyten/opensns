...
class IndexController extends BaseController
{
    ...
    public function ranking()
    {
        ...
        $tag = 'rank_list_ranking_' . $uid;
        $rankList = S($tag);
        if ($rankList === false) {
            $user_fans_list = $memberModel->where(array('status' => 1))->field('uid,fans')->order('fans desc,uid asc')->limit($limit)->select();
            ...
            foreach ($user_fans_list as &$u) {
                $temp_user = query_user(array('nickname', 'avatar32'), $u['uid']);
            ...
            $user_list = $memberModel->where(array('status' => 1))->field('uid,con_check')->order('con_check desc,uid asc')->limit($limit)->select();
            ...
            $user_total_list = $memberModel->where(array('status' => 1))->field('uid,total_check')->order('total_check desc,uid asc')->limit($limit)->select();
            ...
            $user_score_list = $memberModel->where(array('status' => 1))->field('uid,score1')->order('score1 desc,uid asc')->limit($limit)->select();
            ...
            $mapFans['fans'] = array('gt', $userScore['fans']);
            $fansRank = $memberModel->where($mapFans)->count();
            $fansRank += 1;

            $mapConCheck['con_check'] = array('gt', $userScore['con_check']);
            $conCheckRank = $memberModel->where($mapConCheck)->count();
            $conCheckRank += 1;

            $mapTotalCheck['total_check'] = array('gt', $userScore['total_check']);
            $totalCheckRank = $memberModel->where($mapTotalCheck)->count();
            $totalCheckRank += 1;

            $mapScore['score1'] = array('gt', $userScore['score1']);
            $mapScore['status'] = 1;
